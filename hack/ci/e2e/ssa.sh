...
echo ">>> Run client-side apply upgrade test"
set -euo pipefail
...
echo ">>> Run SSA install with CSA upgrade test"
set -euo pipefail
...
echo ">>> Run CSA install with SSA upgrade test"
set -euo pipefail
...
kubectl -n helm-system delete -f config/testdata/server-side-apply/$test_name.yaml
echo ">>> Run SSA to CSA field removal test"
set -euo pipefail
...
kubectl -n helm-system delete -f config/testdata/server-side-apply/$test_name.yaml
echo ">>> Run server-side apply test"
set -euo pipefail
...
echo ">>> Run server-side apply rollback test"
set -euo pipefail
