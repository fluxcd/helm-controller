...
- `.name` (Required): The name of the install strategy to use. One of
  `RemediateOnFailure`, `RetryOnFailure` or `RetryWithUpgradeOnFailure`.
...
- `.retryInterval` (Optional): The time to wait between retries of failed
  releases when the install strategy is set to `RetryOnFailure` or to
  `RetryWithUpgradeOnFailure`. Defaults to `5m`. Cannot be used with
  `RemediateOnFailure`.
...
The `RetryOnFailure` strategy will retry failed installs in a regular
interval defined by the `.spec.install.strategy.retryInterval` field,
applying uninstalls as remediation between each attempt.

The `RetryWithUpgradeOnFailure` strategy will retry failed installs
with upgrade actions in a regular interval defined by the
`.spec.install.strategy.retryInterval` field, without applying any
remediation between each attempt.
