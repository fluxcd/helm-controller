...
func Uninstall(_ context.Context, config *helmaction.Configuration, obj *helmv2.HelmRelease, opts ...UninstallOption) (*helmrelease.UninstallReleaseResponse, error) {
	uninstall := newUninstall(config, obj, opts)
	return uninstall.Run(obj.GetReleaseName())
}

func newUninstall(config *helmaction.Configuration, obj *helmv2.HelmRelease, opts []UninstallOption) *helmaction.Uninstall {
