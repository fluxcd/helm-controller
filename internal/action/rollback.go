...
func Rollback(config *helmaction.Configuration, obj *helmv2.HelmRelease) error {
	rollback := newRollback(config, obj)
	return rollback.Run(obj.GetReleaseName())
}

func newRollback(config *helmaction.Configuration, rel *helmv2.HelmRelease) *helmaction.Rollback {
