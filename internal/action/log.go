...
// LogV returns an action.DebugLog that logs to the given logr.Logger
// with the given verbosity level.
func LogV(log logr.Logger, level int) helmaction.DebugLog {
	return func(format string, v ...interface{}) {
		log.V(level).Info(fmt.Sprintf(format, v...))
	}
}
