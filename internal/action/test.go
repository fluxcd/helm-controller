...
func Test(_ context.Context, config *helmaction.Configuration, obj *helmv2.HelmRelease, opts ...TestOption) (*helmrelease.Release, error) {
	test := newTest(config, obj, opts)
	return test.Run(obj.GetReleaseName())
}

func newTest(config *helmaction.Configuration, obj *helmv2.HelmRelease, opts []TestOption) *helmaction.ReleaseTesting {
