...
func Test(_ context.Context, config *helmaction.Configuration, obj *helmv2.HelmRelease) (*helmrelease.Release, error) {
	test := newTest(config, obj)
	return test.Run(obj.GetReleaseName())
}

func newTest(config *helmaction.Configuration, obj *helmv2.HelmRelease) *helmaction.ReleaseTesting {
