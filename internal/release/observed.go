...
func ObservedToInfo(rls ObservedRelease) *helmv2.HelmReleaseInfo {
...
func TestHooksFromRelease(rls *helmrelease.Release) map[string]*helmv2.HelmReleaseTestHook {
	hooks := make(map[string]*helmv2.HelmReleaseTestHook)
	for k, v := range GetTestHooks(rls) {
		var h *helmv2.HelmReleaseTestHook
		if v != nil {
			h = &helmv2.HelmReleaseTestHook{
