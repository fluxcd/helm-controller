...
// provided values.
func NewAtomicRelease(patchHelper *patch.SerialPatcher, cfg *action.ConfigFactory, recorder events.Recorder, fieldManager string, disallowedFieldManagers []string, defaultToRetryOnFailure bool) *AtomicRelease {
...
func (r *AtomicRelease) actionForState(ctx context.Context, req *Request, state ReleaseState) (ActionReconciler, error) {
	...
	case ReleaseStatusDrifted:
		...
		r.eventRecorder.Eventf(req.Object, req.Source, corev1.EventTypeWarning, v2.DriftDetectedReason, eventv1.ActionReconciling, "%s", msg)
