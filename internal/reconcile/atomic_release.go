		...
		default:
			...
			if !r.strategy.MustContinue(next.Type(), previous) {
				...
				if retry := req.Object.GetActiveRetry(); retry != nil {
					...
					conditions.MarkFalse(req.Object, meta.ReadyCondition, "RetryAfterInterval",
						"Will retry %s after %s", next.Name(), retry.GetRetryInterval().String())
			...
			if r.strategy.MustStop(next.Type(), previous) {
				...
				if retry := req.Object.GetActiveRetry(); retry != nil {
					conditions.Delete(req.Object, meta.ReconcilingCondition)
					conditions.MarkFalse(req.Object, meta.ReadyCondition, "RetryAfterInterval",
						"Will retry %s after %s", retry.GetAction(), retry.GetRetryInterval().String())
					return ErrRetryAfterInterval
				}
...
func (r *AtomicRelease) actionForState(ctx context.Context, req *Request, state ReleaseState) (ActionReconciler, error) {
	...
	case ReleaseStatusFailed:
		...
		if retry := req.Object.GetActiveRetry(); retry != nil {
			switch retry.GetAction() {
			case v2.ReleaseActionInstall:
				log.V(logger.DebugLevel).Info("uninstalling failed release")
				return NewUninstallRemediation(r.configFactory, r.eventRecorder), nil
			default: // action == v2.ReleaseActionUpgrade
