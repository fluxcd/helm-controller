...
// status.
func observeInventory(obj *v2.HelmRelease, src sourcev1.Source, chart *helmchart.Chart, getter genericclioptions.RESTClientGetter, recorder events.Recorder) storage.ObserveFunc {
	return func(rls helmrelease.Releaser) {
		rlsTyped, ok := rls.(*helmreleasev1.Release)
		if !ok {
			return
